...
app.get('/api/sse/:device_code', async (req, res) => {
  ...
  if (!dev) return;
  res.status(200);
  const isHttp2 = Number(req.httpVersionMajor || 1) >= 2;
  // Nota: en HTTP/2 NO se pueden enviar headers tipo "Connection".
  // Railway/Cloudflare pueden servir HTTP/2 al navegador; si enviamos ese header,
  // Chrome lanza net::ERR_HTTP2_PROTOCOL_ERROR aunque el status sea 200.
